...
2. **网络辅助的拥塞控制。**在网络辅助的拥塞控制方法中, 路由器会向发送方提供关于网络中拥塞状态的信息。一般来说有两种方式。第一种是直接反馈, 路由器直接向发送方发送通知, 告知发送方自己已经拥塞了; 另一种方式更为普遍, 路由器会标记发送方流向接收方的分组, 接收方受到分组后, 便向发送方发送信息, 告知发送方网络已拥塞。
