...
**本节只讨论根据目的地转发分组的情况**。通用分组转发参见 [4.4 通用转发和 SDN](##通用转发和SDN)
...
## 通用转发和 SDN
由于第二层交换机、第三层路由器等中间盒数量的剧增，每种中间盒都有不同的功能、硬件、软件和管理界面，给网络管理带来了极大的负担。因此，我们寻求一种统一的方法，能够实现网络层功能以及部分链路层功能，从而替代繁多的中间盒，这就是**通用转发**。

通用转发本质上也是基于目的地的转发。
一般的基于目的地的转发（如 IP 数据报的转发）可以抽象为两部分：
1. 匹配：查找目标 IP 地址。
2. 动作：将数据报送到指定的端口。

通用转发在这两步上都进行了扩展。
1. 匹配：可以对多个协议栈及首部进行匹配。
2. 动作：丰富了可执行的动作，包括转发分组、负载均衡、重写首部值、选择性的丢弃分组（防火墙）等。

为了便于区分，我们将实现通用转发的设备称为**分组交换机**。

todo 4-28

可以看到，和一般基于目的地的转发设备一样，在分组交换机中，也存在着一张**匹配加动作表**。
每台分组交换机的匹配加动作表都是通过远程控制器计算、安装和更新的。**因此由分组交换机组成的网络属于 SDN 网络**。

### OpenFlow
`OpenFlow` 是通用转发的先驱，目前已得到了比较广泛的认可。其当前版本和早期版本见于 [ONF 2016](todo 链接)。

后面的对于通用转发的讨论会基于 OpenFlow 1.0 进行。

在 OpenFlow 中，匹配加动作表被称为`流表（flow table）`。其中每个表项都包括：
- 首部字段值的集合：入分组将与之进行匹配，匹配不上的分组会被丢弃或发往远程控制器做进一步处理。
- 计数器集合：已与该表项匹配的分组数以及表项更新时间。
- 动作集合：当分组匹配上该表项之后，需要执行的动作。

#### 匹配
todo 图 4-29

上图展示了 OpenFlow 1.0 流表的分组匹配字段，共计 11 个。

其中入端口指的是分组交换机上分组输入的端口。
流表项中也存在通配符，例如 IP 地址 192.168.\*.\*。
每个流表项也存在优先级，如果一个分组匹配多个流表项，选定的匹配和动作将是拥有最高优先级的那个。

可以看到，OpenFlow 可以对多个协议及其首部字段进行匹配。

但其并不完全支持三个协议所有的首部字段，对于一些协议中特别复杂的功能，OpenFlow 进行了舍弃，以保证系统不至于太过臃肿。

#### 动作
动作决定了应用于分组上的处理。一个流表项可以包含多个动作。

流表项中的动作包括：
1. 转发。
2. 丢弃。
3. 修改字段：在转发到输出端口之前，分组首部中除入端口以外的字段均能进行修改。

### SDN
OpenFlow 是 SDN 网络的典型例子。通过远程控制器修改分组交换机中的匹配动作表，我们可以很方便的定义整个网络的行为模式。
